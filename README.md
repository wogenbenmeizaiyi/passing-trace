...
星期八是一个面向个人的身份与活动记录平台。它以 Android App 为主要入口，支持记录文本、图片、视频、文件、分类标签与地点；电脑端可以通过手机扫码批准登录，并使用 Web 客户端查看记录、与个人 AI 助手对话。代码中的 `PassingTrace` 继续作为内部技术标识，避免破坏程序集、包名、协议和已有数据。
...
| 客户端 | 定位 | 主要用途 |
| --- | --- | --- |
| Android App | `passingtrace-mobile/` | 注册与登录、记录生活、故事线创建/阅读/快捷补充、上传附件、定位地点、AI 问答、扫码批准电脑登录 |
| Web | `passingtrace-web/` | 电脑端记录管理、Vue Flow 故事线完整编辑、AI 问答、分类与地点编辑、发起扫码登录 |
