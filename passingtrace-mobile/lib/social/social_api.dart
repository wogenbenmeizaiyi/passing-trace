...
class SocialApi {
  ...
  Stream<int> changes({int after = 0}) async* {
    var cursor = after;
    while (!_closed) {
      try {
...
class SocialFeed extends ChangeNotifier {
  SocialFeed(this.auth, this.session);
  final AuthService auth;
  final AuthSession session;
  ...
  void start() {
    ...
    final api = SocialApi(auth, session);
    _streamApi = api;
    ...
    _subscription = api.changes(after: _cursor).listen((id) {
      _cursor = id;
  ...
  Future<void> _refresh(SocialApi api) async {
    try {
      ...
      if (!_disposed && _streamApi == api) {
        unread = (data['count'] as num).toInt();
        notifyListeners();
      }
    } catch (_) {}
  }
